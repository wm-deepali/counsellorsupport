						...
						<div class="content">
							...
							<table class="table">
								...
								<tr>
									...
									<td class="data"><span><?= $user[0]->service_name ?></span>
									</td>
								</tr>
								<tr>
								...
								<tr>
									...
									<td class="data"><img src="<?= base_url() ?><?= $user[0]->image ?>" alt="Attachment" class="img-fluid">
									</td>
								</tr>
					...
					<div class="col-sm-4">
						<div class="payment">
							...
							<div class="text-center payment-button">
								<button type="button" id="proceed_payment" class="pay-btn" data-target="#pay_now" data-toggle="modal">Proceed to Payment <i class="fas fa-chevron-circle-right"></i></button>
							</div>
						</div>
					</div>
									...
									<div class="payment-modal">
										<div class="row">
											<div class="col-sm-6 border-1r text-center">
												<h4>Scan QR Code</h4>
												<img src="<?= base_url() ?>images/qr.png" class="img-fluid">
											</div>
											<div class="col-sm-6 align-self-center text-center">
												<form action="<?= $action ?>" method="post" name="payuForm">
													...
													<input name="amount" type="hidden" value="1" id="amoint"> 
													<input name="firstname" type="text" id="firstname" value="<?php echo (empty($user[0]->name)) ? '' : $user[0]->name; ?>" />
													<input name="email" type="hidden" id="email" value="<?php echo (empty($user[0]->email)) ? '' : $user[0]->email; ?>">
													<input name="phone" type="hidden" value="<?php echo (empty($user[0]->mobile)) ? '' : $user[0]->mobile; ?>">
													<input name="productinfo" value="<?php echo (empty($user[0]->service_name)) ? '' : $user[0]->service_name ?>" type="hidden" id="service_name">
													<input name="surl"  type="hidden" value="<?= base_url(); ?>front/success">
													<input name="furl"  type="hidden" value="<?= base_url(); ?>front/failure">
													<input type="hidden" name="service_provider" value="payu_paisa" id="service_provider">
													<input type="hidden" name="appoint_id" value="<?php echo (empty($user[0]->id)) ? '' : $user[0]->id ?>" id="appoint_id">
				...
				$("#btn-save").click(function() 
				{
					...
					var amount = $('#amount').val();
						jQuery.ajax({
							...
							dataType: 'html',
							data: {txnid: txnid, hash: hash, appoint_id: appoint_id, service_provider:service_provider},
